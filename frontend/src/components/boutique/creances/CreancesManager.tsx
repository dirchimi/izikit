...
export default function CreancesManager() {
  ...
  const t = useT();
  const [tab, setTab] = useState<'debtors' | 'repayments'>('debtors');
  ...
  return (
    <>
      ...
      {/* Onglets : clients débiteurs ↔ remboursements reçus (tous clients). */}
      <div className="border-border flex items-center gap-1 border-b px-4 pt-3 md:px-8">
        {(['debtors', 'repayments'] as const).map((tb) => (
          <button
            key={tb}
            type="button"
            onClick={() => setTab(tb)}
            className={`font-body -mb-px border-b-2 px-3 py-2 text-sm font-semibold ${
              tab === tb
                ? 'border-primary text-primary'
                : 'text-muted-foreground hover:text-foreground border-transparent'
            }`}
          >
            {t(tb === 'debtors' ? 'creances.tab.debtors' : 'creances.tab.repayments')}
          </button>
        ))}
      </div>

      {tab === 'repayments' && <RepaymentsView />}
              ...
              <div className="border-border flex flex-col gap-2 border-b px-4 py-3">
                <div className="border-border bg-input focus-within:border-primary flex items-center gap-2 rounded-md border px-3 py-2">
                  ...
                  <input
                    ...
                    aria-label={t('common.search.client')}
