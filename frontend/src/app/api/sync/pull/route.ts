...
// Returns the 7 org-scoped domain tables + a `serverTime` cursor the
// client stores (`meta.lastPull`) for its next incremental pull.
//
// Filtering: `updatedAt > since` per table when `since` is provided,
// otherwise a full org snapshot. All 7 models (`Product`, `Customer`,
// `Sale`, `Receivable`, `Expense`, `Document`, `StockMovement`) carry an
// `@updatedAt` column (Task 1.2 schema prerequisite — Sale/Expense/
// Document/StockMovement gained theirs in migration `35_sync_updated_at`),
// so the same filter shape applies uniformly across the Promise.all.
//
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  return withRequestContext(ctx, async () => {
    ...
    const [products, customers, sales, receivables, expenses, documents, stockMovements] =
      await Promise.all([
