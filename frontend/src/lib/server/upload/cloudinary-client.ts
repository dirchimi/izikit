...
export async function uploadBuffer(
  ...
  // Surface the validated MIME as free-form context. NB: `context` (free-form
  // key=value), NOT `metadata` — Cloudinary's `metadata` param requires
  // structured-metadata fields pre-created in the account and rejects the
  // upload otherwise (opaque 502 at the route boundary).
