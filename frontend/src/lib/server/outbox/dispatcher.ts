...
async function dispatchEvent(deps: OutboxDispatcherDeps, event: OutboxEvent): Promise<void> {
  switch (event.kind) {
    ...
    case 'email.org_invitation': {
      // Émis par /api/org/invitations ; rendu via orgInvitationEmail().
      if (!deps.emailQueue) throw new Error('email queue not configured');
      const { orgInvitationEmail } = await import('../auth/email-templates');
      const { to, orgName, inviterEmail, link, expiresAt } = event.payload;
      const tpl = orgInvitationEmail({ to, orgName, inviterEmail, link, expiresAt });
      await deps.emailQueue.enqueue({ to, subject: tpl.subject, html: tpl.html });
      return;
    }
    default: {
