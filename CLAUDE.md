...
**Offline-first (boutique POS)** — the app works with no network and syncs when connectivity returns. The model: a Dexie (IndexedDB) local mirror of the org-scoped tables + a client-side write outbox + a sync engine that drains it. Every offline write is stamped with a client-generated cuid2 (`newId`/`newOpId` in [frontend/src/lib/offline/ids.ts](frontend/src/lib/offline/ids.ts)) that doubles as the row's own id (sales/customers/etc.) or as `clientOpId` (aggregate mutations like repay/adjust) — that same value is the idempotency key the server dedupes on when the queued write is finally replayed, possibly hours later and possibly twice.

Key files, all under [frontend/src/lib/offline/](frontend/src/lib/offline/):
- `db.ts` — Dexie schema: server-mirrored tables (products, customers, sales, saleItems, receivables, repayments, expenses, documents) plus offline-only tables (`outbox`, `session`, `meta`, `conflicts`).
- `ids.ts` — `newId`/`newOpId` cuid2 generator (see above).
- `outbox.ts` — queue CRUD only (`enqueue`/`listPending`/`markSyncing`/`markDone`/`markConflict`/`markError`); does not touch the network.
- `sync-engine.ts` — `drainOutbox()`: reads pending rows in `seq` (FIFO) order and POSTs each through the shared `api()` wrapper (never a raw `fetch`, so CSRF + 401 auto-refresh still apply). A network error stops the drain and resets the row to `pending`; a business rejection marks the row `error`/`conflict` and drain continues; a 2xx marks it `done`.
- `sync-triggers.ts` — decides WHEN to call `drainOutbox()` (on install, on the browser `online` event, every 30s while online with a non-empty queue, and on-demand via `triggerDrain()`).
- `useSyncStatus.ts` / `useLocalResource.ts` — `dexie-react-hooks` `useLiveQuery` composition for the sync badge and for reading local data reactively; both are `'use client'` modules.
- `pull.ts` — seeds/refreshes the local mirror from `GET /api/sync/pull` (one full-batch round trip, no per-resource filtering).
- `session.ts` — persisted auth snapshot so the app can answer "who is logged in" offline (7-day sliding window, refreshed on every successful online `/api/auth/me`).
- `mutations.ts` — the offline write path (`createSaleOffline`, etc.): applies an optimistic local effect (e.g. local stock decrement) and enqueues the equivalent online request onto the outbox; the server is authoritative and recomputes totals/numbers at sync.
- `purge.ts` — best-effort local history retention (default 60 days) for append-only tables; never touches a row that hasn't finished syncing, never touches working-set tables (products/customers/receivables/documents/meta/outbox/session).

Server side: [frontend/src/lib/server/idempotency.ts](frontend/src/lib/server/idempotency.ts) exports `withIdempotency(tx, { organizationId, clientOpId, endpoint }, fn)` and [frontend/src/app/api/sync/pull/route.ts](frontend/src/app/api/sync/pull/route.ts) serves the full-batch pull.

Invariants a future agent MUST preserve:
1. **`withIdempotency` MUST stay inside `withTxRetry`.** On a real replay race, the loser's `tx.offlineOperation.create` throws Postgres `P2002`; Prisma interactive transactions don't support in-tx savepoints, so the error is left to propagate out of `withIdempotency` and out of the whole `prisma.$transaction(...)` — the caller's `withTxRetry` wrapper retries the entire transaction, and on retry the up-front `findUnique` cleanly finds the winner's row. Catching `P2002` locally instead of retrying the whole tx is a regression.
2. **Business rejections THROW (roll back); only success is memoized.** A rejected write (e.g. `NOT_FOUND`) must never be persisted as the `OfflineOperation` row's `resultJson` — a later replay after the underlying state changes (e.g. the record now exists) must be free to succeed. Only a `kind: 'OK'` result gets memoized.
3. **Sales record + flag stock conflicts at sync — they never reject on stock.** If server-side stock can't cover an offline sale, the sale is still recorded, the shortfall is reported per line-item in the response's `stockConflicts` array, and the decrement is bounded at 0 — a stale local stock snapshot must never turn into a lost sale. `PRODUCT_NOT_FOUND` (the product itself was deleted) is the one case that's still a hard rejection.
4. **Offline mutations go through the sync-engine, not `api.ts`'s retry.** `frontend/src/lib/api.ts` still never retries mutating verbs (see its own invariant above) — replay-on-reconnect is `sync-engine.ts`'s job, driven by the outbox, not a retry loop bolted onto `api()`.
5. **The client entry-timestamp is bounded server-side.** Offline sales/expenses can carry a client-supplied occurred-at time (the device may create the record while offline, hours before it syncs); the server clamps/validates it (see [frontend/src/lib/server/time/client-entry-timestamp.ts](frontend/src/lib/server/time/client-entry-timestamp.ts)) rather than trusting the client clock outright.
6. **A client-only module (Dexie / `dexie-react-hooks` / anything touching `indexedDB`/`window`/`navigator`) must never be imported into a Server Component or any module that runs during SSR/prerender** — it will break `next build`. Every file above that touches Dexie directly is `'use client'` or is only ever imported from one.

Known limitations (accepted, not bugs): no delete-tombstone — a deleted product lingers in the local mirror until the next full pull, so an offline sale against an already-deleted product is flagged `PRODUCT_NOT_FOUND` at sync rather than silently lost; the app shell is only cached by the browser on a full navigation (no service-worker precache); supplier-debt payment and product create/edit/delete are NOT available offline; local `stockMovements` rows written by an offline sale/cancel stay unsynced (informational only — the server recomputes its own movements at sync).
